...
> ⚠️ **重要说明**：WoWorld 处于**设计+编码并行阶段**。设计文档中的伪代码和数据结构示例用于阐明设计理念，实际 Rust 实现可能根据工程发现重构。`woworld/` 已进入**轨A 阶段三·MC体素+Clipmap LOD+海洋**——5 crate workspace，核心类型/trait/空间索引/世界生成(MC+Clipmap+LOD)/大气氛围/海洋渲染/Godot 桥接全部就位。
>
...
│   │       ├── entity_index.rs #   稀疏网格实体索引
│   │       ├── visibility.rs   #   DDA 射线可见性查询
│   │       └── event_bus.rs    #   环形缓冲区事件总线
│   ├── woworld_worldgen/       # 程序化世界生成 (MC + Clipmap LOD + 海洋)
│   │   └── src/
│   │       ├── lib.rs          #   导出 HeightfieldTerrain, ChunkManager, ClipmapManager, TerrainMeshData
...
- Godot 项目代码在 `woworld/` 目录下（Rust workspace + Godot 项目，轨A 阶段三·MC体素+Clipmap LOD+海洋）
