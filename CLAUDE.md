...
**设计→编码过渡中**：技术方案历经 001→015（v1.0）→016（v2.0）→018（v3.0）→**v4.0（当前权威，经技术栈全量审计升级）** 演进。开发路线图已启动——设计文档与代码脚手架并存。路线图功能由 `DEPENDENCY_GRAPH.md` + `DEVELOPMENT_STATUS.md` + 冲刺提案共同承担（旧四轨计划已于 2026-06-25 归档）。

> ⚠️ **重要说明**：WoWorld 处于**设计+编码并行阶段**。设计文档中的伪代码和数据结构示例用于阐明设计理念，实际 Rust 实现可能根据工程发现重构。`woworld/` 已进入**轨A·MC体素+Clipmap LOD+海洋**——4 crate workspace，核心类型/trait/世界生成(MC+Clipmap+LOD)/大气氛围/海洋渲染/Godot 桥接全部就位。
>
...
> 📘 **三份文件的分工**: **CLAUDE.md** = 项目大局观 + 工作约定。**[[CLAUDE-INTERFACES.md]]** = 跨模块契约完整参考。**[[WORKFLOW.md]]** = 开发流程全景图（冲刺生命周期、质量门、分诊协议、文件速查）。
...
近期关键变更：**CHG-053**（Godot 4.7·12子系统）→ **CHG-054**（世界生成 v2.1）→ **CHG-055/056**（存档系统 v1.0→v2.0）→ **CHG-057**（NPC认知 v1.1·PatternExpression数学地基）→ **CHG-058**（NPC认知系统自审修正）→ **CHG-059**（NPC认知v1.1全模块传播审计）→ **CHG-060**（开发路线图优化·四轨重定义·孤儿接口修复）→ **CHG-061**（轨C孤儿接口修复·CHG-063前置）→ **CHG-062**（UI与UX系统创建）→ **CHG-063**（玩家系统新建·6篇~1,448行·28-玩家系统登记）。
...
| 文件 | 角色 |
|------|------|
| `woworld-dev-plan/DEPENDENCY_GRAPH.md` | 地图——层 0→1→2→3→4 实现顺序 |
| `woworld-dev-plan/DEVELOPMENT_STATUS.md` | 当前位置——每个模块 🔴🟡🟢 + WIP |
| `woworld-dev-plan/sprint-proposals/` | 下一步——§8 格式冲刺提案 |
...
项目已进入轨A·GPU-Driven Clipmap+海洋。Rust workspace 含 4 个 crate，**72 个测试全部通过**。
...
> **当前状态**（2026-07-04 验证）：`cargo check --workspace` 通过。`cargo test` 72 个测试全部通过, clippy 零警告。GPU-Driven Clipmap 8 层 LOD + Gerstner 海洋 + 昼夜循环 + 5 群系系统 + OceanProvider trait + Transvoxel 骨架就位。
>
> **★ Sprint 029 — 海洋**: `OceanProvider` trait (woworld_core) + `HeightfieldOcean` + DEPTH_TEXTURE per-fragment 海深色变 + 水下 Environment 参数调制。
> **★ Sprint 030 — Transvoxel 骨架**: `CaveDensity` 3D Worley 洞穴层 + `transvoxel_extract()` + 过渡表恢复。密度栈现含 2 层。Godot 集成留到 Sprint 031。
> **★ 背面剔除修复**: clipmap.rs 5 处三角形索引统一为 CW 绕序 → `cull_back` 正常。
> **★ 地形光照修复**: terrain.gdshader 去 `unshaded` + 梯度法线 + view-space NORMAL。
> **★ 大陆尺寸**: continent_scale 10x → 大型大陆替代小群岛。
...
- **开发宪法系统**: `woworld-dev-plan/` — CONSTITUTION.md（元规则层·v1.5）+ DEVELOPMENT_STATUS.md（全局状态）+ DEPENDENCY_GRAPH.md（依赖图）+ sprint-proposals/ + handoff/。定义冲刺工作流和决策规则。流程全景见 WORKFLOW.md。
...
| 🟡 **已知偏离** | 已在 `DEVELOPMENT_STATUS.md` 或冲刺提案中追踪。 | 引用追踪 ID。不重复调查。 |
