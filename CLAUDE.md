...
- `开发阶段/` — 实现规格/工程分析（版本号如 ver1.01，权威规格）
  - `NPC活人感模块/` — NPC系统权威规格（**ver2.0**，Rust伪代码，覆盖半自动战斗/载具/天象/蓝图/海洋。原ver1.01 GDScript版已废弃）
...
| 编号 | 内容 |
|------|------|
| **018** | [**正式技术栈方案 v3.0**](WoWorld-Design/参考文档/018-正式技术栈方案v3-20260610/) ← **★ 当前权威方案** |
| **017** | [开发阶段测试记录](WoWorld-Design/参考文档/017-开发阶段测试记录-20260610/) — 方法论+50份双视角测试报告（每Phase 5次，7 Phase共35份v3目标） |
| 016 | 正式技术栈方案 v2.0（已被018取代，留作参考） |

详见 `参考文档/README.md`

## 技术栈（v3.0 权威方案）

> 📘 **权威技术方案见 `参考文档/018-正式技术栈方案v3-20260610/`**。以下为概要。
...
- **战斗**: **半自动**——玩家战斗AI = NPC战斗AI = 同一套Rust代码。玩家仅做高层决策（切换战斗风格/关键技能/队伍命令/投降逃跑）。电影级自动战斗
...
- **技术决策**：以 `参考文档/018-正式技术栈方案v3-20260610/` 为权威依据。所有 001-016 为历史演进存档，017 为测试方法论
