...
**当前规格版本**: v4.0（经技术栈全量审计升级）。模块累计 **27 个独立系统** + 1 个深化子模块（家具与放置物品）+ 交互配方表系统（物品获取物理统一入口，007 §九）+ 存档系统 v2.0（CHG-055/056），~120,000行正式开发规格。★ 2026-06-22 Loop Audit 全量审计完成（10轮·25模块全覆盖·28项人工裁决已解决）。最新 CHG 序列见 `Change/` 目录。
...
- `开发阶段/` — 正式开发规格（权威规格）。27 个独立系统 + 1 个深化子模块（[[WoWorld-Design/Happy Game/开发阶段/物品系统/家具与放置物品/README|家具与放置物品]]），分层如下：

| 层级 | 模块 | 角色 |
|------|------|------|
| **全局基础** | `技术栈方案/`（★ v4.0 权威）、`模块接头总览/`（接口地图）、`存档系统/`（★ v1.0 新建 — 本次） | 所有模块的决策依据、持久化契约和接口枢纽 |
...
| **氛围** | `大气与氛围系统/`（★ v1.0 新建 — CHG-053） | 环境氛围规则引擎——群系×时间×天气×季节。消费 WeatherQuery（★ 2026-06-22 双向引用已建立） |
| **建造** | `建筑模块/`、`载具系统/` | NPC 和玩家对世界的物理改造 |
| **玩家辅助** | `小精灵系统/`（★ v1.0 新建 — CHG-052） | 可选辅助伙伴——引导/问答/LLM对话 |
| **存档/持久化** | `存档系统/`（★ v1.0 新建 — CHG-055 · **v2.0 审计修正 — CHG-056**） | 世界状态的快照相机——LMDB单文件多named_db·SaveableModule trait v2.0（14方法·4必覆·10默认）·LoadContext渐进加载·崩溃恢复三件套·惰性迁移·死亡优先级 |

### 关键架构关系：技能系统 ↔ 物理原子层

这两个系统容易误读为冲突（离散技能名 vs 连续物理参数），实际是正交维度：

- **物理原子层**（`NPC活人感模块/08-NPC行动涌现与分类/`）：AgentSnapshot 连续参数决定**身体能不能执行**物理动作（force_check）。35个物理原子纯物理计算，零领域知识。**★ v1.1 (2026-06-21)**: AgentSnapshot 认知段 5→8 字段——新增 CognitiveTide 3字段（cognitive_load / rumination_pressure / mind_quietude），每决策周期映射，影响战斗/魔法/社交/经济所有原子执行质量。总量 ~108 bytes·26 fields·单 cache line。
...
**关键类型所有权约定**（★ 2026-06-21 深度审计确立）：
- 所有 ID 类型（`ItemDefId`·`SkillId`·`EntityId`·`ProfessionTagId` 等）统一定义在 `woworld_core`——零依赖 crate，各模块平等引用
- TOML 数据文件（ProfessionTag·CraftingRecipe·BuildingComponent 等）为中性数据——各消费模块通过 `include_str!()` 平等加载，无 Owner 模块
...
### 世界生成 ←→ 存档系统（★ CHG-055 关键架构变更）

**P13 不再写 LMDB。** LMDB 写入权从世界生成完全移交给存档系统。P13 输出纯内存 `ValidatedWorldState`，存档系统在 P13 完成后创建 Initial 存档。这意味着：
- 世界生成模块不再依赖 LMDB crate——P0-P13 是纯计算
- 所有持久化走唯一路径：`SaveableModule::snapshot_dirty() → SaveSystem → LMDB`
- 历史模块的"独立 LMDB 文件"也统一为存档内 named_db

### 存档系统（`存档系统/` — ★ v1.0 新建）

世界状态的持久化层——不参与游戏模拟，只负责存和取。**SaveableModule trait** 是模块与存档系统的唯一契约（v2.0: 14 方法·4 必覆·10 默认，CHG-056 从 v1.0 8方法扩展）。SaveSystem 零模块耦合。

**核心设计决策**：
- **全量快照 + 脏数据增量**（非"种子零存储"）。初始状态全量存，后续只写脏数据
- **单文件 LMDB + 多 named_db**（单写事务覆盖所有模块 = 跨模块原子写入）
- **种子从容错手段重新定位**：存档包含完整数据，种子在损坏时可重建种子可重现层
- **`snapshot_dirty(&self)` + `confirm_snapshot_written(&mut self)` 两阶段**：`dirty_since` 时间戳消除快照后修改的竞态
- **模块自治**：序列化格式、数据老化策略、加载策略全由各模块自决
- **崩溃恢复三件套**：临时文件+原子重命名 / 覆盖前自动备份 / session.lock 会话检测
- **存档槽设计**：手动无上限 + 自动轮转(3)·快存(1)·退出(1)·Initial(1)
- **玩家死亡继承**：PendingInheritance → 血缘继承或灵魂转世（随机 NPC）
- **Mod 兼容**：删 Mod → 冻结/替换降级。世界发现用目录扫描（文件系统即真相）
- **LoadContext 渐进加载**：`load(&mut self, ctx: &LoadContext)` —— 提供 `txn: RoTransaction` + `create_txn` 工厂，模块按需惰性加载
- **惰性迁移**：ConsumableEffect 等跨模块迁移在 `load()` 中批量转换，零跨模块协调

详见 `开发阶段/存档系统/README`（7 篇正式规格 + 跨模块依赖清单）。

### `Change/` — 设计变更追踪

> ⚠️ **Change 文件夹约定**：编号大的、较新的 CHG 覆盖编号小的、较旧的。Change 文件夹内容仅供参考——**非每次大更改都写入 Change**。以 `开发阶段/` 文件夹的实际内容为权威。

按 `CHG-XXX-简短描述-YYYYMMDD.md` 命名。CHG-001~006 早期变更，CHG-007~013 审计与重构，CHG-014~019 地基模块，CHG-022~033 业务系统（经济→模型动作物理），CHG-034~044 技术栈审计+新模块，CHG-045 世界生成 v2.0 重构，CHG-046 植被系统架构升级，CHG-047 全模块交叉审计，CHG-048 交叉审计修复，CHG-049 LOD 架构全面深化，CHG-050 家具与放置物品系统 v1.0 创建（物品系统子模块），CHG-051 交互配方表系统（物品获取物理统一入口），**CHG-052 玩家游玩内容全貌设计（36问13编34章——小精灵系统+方阵统计+角色管理+大日志多镜头+复合原子注册+法律涌现分析归档）**、**CHG-053 Godot 4.7 技术栈升级与设计深化（引擎升级+画面渲染管线+语音合成+氛围系统+wear维护+面部表情+Bark语声——12子系统设计）**、**CHG-054 世界生成 v2.1（P8 Phase B 五Pass重构+P9a 事件驱动优化+RelationshipNorms 集成）**、**CHG-055 存档系统 v1.0 创建（全量快照+脏增量·SaveableModule trait·LMDB单文件多named_db·崩溃恢复三件套·版本迁移·Mod兼容——27模块累计·6篇正式规格·9文档联动修改）**、**CHG-056 存档系统深度审计与修正（10轮迭代审计·24问题·trait v2.0 14方法·LoadContext渐进加载·惰性迁移·死亡优先级·UUID去重·流式写入·Initial直写路径）**。详见 `Change/README.md`。

**`Change/hand/`** — 用户直接设计反馈。修改涉及的设计决策时，需检查此目录是否有相关意见。

### `参考文档/` — 参考性设计文档
按 `NNN-简短描述-YYYYMMDD` 格式组织。001-015 已归档至 `第一部分-设计演进存档-20260610/`。活跃文档 017-021 + 031-037 覆盖测试记录、技术栈、各系统设计探讨、引擎分析、**世界生成重构跨模块补充需求(037)**、**法律涌现与执法机制分析(035)**、**玩家游玩内容全貌设计(036)**。详见 `参考文档/README.md`。
...
- **引擎**: Godot 4.7 | **模拟语言**: Rust (stable 1.80+) — GDExtension 集成
...
- **画面**: 3D低多边形 + **Toon 2-tone cel渲染**（硬切亮暗面 + Fresnel rim light + SSAO + Bloom + AgX + 大气透视）。512²面部图集+shader合成驱动表情。**★ CHG-053**
...
- **世界生成**: 编排器模式——15阶段管线(P0-P13 + P2.25植被覆盖)。结构层全量预生成(NPC/权力/经济/历史)，细节层按Chunk渐进(内饰/植物/动物)。VMC双层空间架构(75K个1km²植被宏观Chunk·运行时32m TC确定性展开)。Bootstrap哲学——初始订单簿/初始权力边=已运行千年世界的当前快照，非破坏涌现。协同生成——NPC+家族+历史事件同时产生。五Pass混合构造(祖先创建·配偶池合并·社会关系初始化含同性多配偶非婚生·已故子女·DAG验证)·NpcAnchors(家/工作/产权)·整数规划职业世系·KnowledgeSeed事件驱动传播·因果事件图·死因装饰·P13生成后完整性校验。★ P13 输出纯内存 ValidatedWorldState，持久化由存档系统接管。详见 `开发阶段/世界生成/`（15篇·001-014+README · v2.1 + CHG-046）
- **载具**: 魔力驱动火车/帆船。移动参考系。NPC集体建造
- **建造/蓝图**: 三途径——编辑器/TOML导入/委托NPC。NPC团体集体施工
- **数据库**: LMDB — 存档系统的存储引擎。单文件 + 多 named_db。全量快照 + 脏数据增量。种子从空间优化重新定位为容错恢复手段。详见 `开发阶段/存档系统/`
...
- **硬件目标**: GTX 1660 SUPER 6GB VRAM。VRAM~4.2GB/6GB（理论估算——待 Godot 原型运行后用 RenderDoc/NVIDIA Nsight 抓帧验证），内存~1.4GB/32GB，帧预算16.7ms（60fps）
...
| 041~050 | 生命周期·NPC原子层·建筑·概念语言地基·世界生成v2.0→v2.1·植被·交叉审计·LOD深化·家具与放置物品 | ★ 世界生成 v2.1：五 Pass 混合构造+社会关系初始化(同性/多配偶/非婚生)+RelationshipNorms |
| 051~052 | 交互配方表·玩家游玩全貌（小精灵/方阵统计/角色管理/大日志多镜头/复合原子注册/法律涌现分析）| 玩家游玩内容 + 法律涌现归档 |
| 053 | 画面渲染管线·语音合成·氛围系统·wear维护·面部表情·Bark语声·大气与氛围模块 | Godot 4.7 升级 + 12 子系统设计 |
| 055 | 存档系统 v1.0（全量快照+脏增量·SaveableModule trait·LMDB单文件多named_db·崩溃恢复·版本迁移·Mod兼容） | 6 篇正式规格·9 文档联动修改·11 轮审计 |
| 056 | 存档系统 v2.0 深度审计修正（trait 8→14方法·LoadContext·惰性迁移·死亡优先级·流式写入） | SaveableModule v2.0·全模块待实施 |
| — | **装备槽位 v1.2**（2026-06-21） | ★ 肩部装备槽位(shoulder_drape解剖条件→围巾/斗篷/披风)·戒指10→4命名手指槽位·容器纯数据层·视觉独立开关 |
| — | **2026-06-21 深度接口审计** | ★ AgentSnapshot v1.1 (CognitiveTide 3字段进入)·ProfessionTagId→woworld_core·孤儿接口汇总·接头保鲜延迟声明·NonVerbalSignal 标注修正 |
| — | **2026-06-22 Loop Audit 全量审计**（10轮·25模块） | ★ AgentSnapshot v1.1 stale 修复 (CLI/009/010)·SaveableModule 接头 v1→v2·DeathCause 25→30 接头同步·技术栈 README v3→v4·CognitiveTide 命名统一 (mind_quietude)·天气↔大气双向桥接·12 README 扩展至≥60行·EventTriggered libido v1.0 日历后备·PhysicalAtom 加入主文档·P2.25 预算对齐·ReligiousReproductionNorms↔FertilityNorms 桥接 |
...
> 📘 **位置**: `开发阶段/模块接头总览/` | **最后填充**: 2026-06-21 — 世界生成 v2.1（五 Pass 混合构造+RelationshipNorms 集成）已刷新。P8 新增消费 FertilityNorms·RelationshipNorms(文化)·ReligiousReproductionNorms(信仰)。P9a 事件驱动优化。
...
模块接头总览/
  ...
  01~26/              — 每模块4文件: 001-接口出口 / 002-接口入口 / 003-变更影响链 / 000-变更日志
                       （26-存档系统 ★ CHG-055 新建）
  变更追踪/           — 待处理变更队列 + 已解决变更日志 + 影响地图
  孤儿接口汇总/       — ★ 2026-06-21 深度审计创建——接口不一致全清单+实质设计问题+修复追踪
```
> ⚠️ **保鲜延迟声明 (2026-06-22 Loop Audit 更新)**: 关键模块接头已更新至 2026-06-22（26-存档系统 v2.0·12-历史 DeathCause 30种·03-世界生成 CHG-054）。剩余模块时间戳待后续增量更新。详见 `.claude/audit-reports/20260622-001/FINAL-SUMMARY.md`。
...
- **数学计算铁律**：任何涉及数学计算（公式求值、参数验证、数值比较、概率推算、单位换算、范围边界检查…），**必须**使用 `python -c "…"` 执行计算，**禁止** LLM "心算"。计算结果需标注来源（Python 执行输出）。设计文档中的关键公式应附 Python 验证脚本，确保文档里的数字是算出来的而非蒙出来的
