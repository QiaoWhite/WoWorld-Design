...
**当前规格版本**: v4.0（经技术栈全量审计升级）。模块累计 **26 个独立系统** + 1 个深化子模块（家具与放置物品）+ 交互配方表系统（物品获取物理统一入口，007 §九），~116,000行正式开发规格。最新 CHG 序列见 `Change/` 目录。
...
- `开发阶段/` — 正式开发规格（权威规格）。26 个子模块 + 1 个深化子模块（[[WoWorld-Design/Happy Game/开发阶段/物品系统/家具与放置物品/README|家具与放置物品]]），分层如下：
...
模块接头总览/
  ...
  01~25/              — 每模块4文件: 001-接口出口 / 002-接口入口 / 003-变更影响链 / 000-变更日志
