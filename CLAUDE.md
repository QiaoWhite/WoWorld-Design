...
项目记忆存储于 `~/.claude/projects/F--Program-Need-The-Game-Dev-The-Development/memory/`，由 `MEMORY.md` 索引。每个记忆一个 `.md` 文件（带 frontmatter: name/description/metadata）。分类：`user`（用户偏好）、`feedback`（反馈纠正）、`project`（项目事实）、`reference`（外部引用）。涉及跨会话需要记住的项目决策、用户反馈、非代码库可推导的背景时写入。
