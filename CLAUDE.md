...
> 📘 **跨模块契约的完整权威参考见 [[CLAUDE-INTERFACES.md]]**（891行）——包含概念所有权、trait 签名、关键约定。修改任何跨模块概念时必须同步维护它。
...
| 新建模块 | 8步流程：参考草稿→跨模块审计→正式规格→关联修改→审查→CHG文档→更新CLAUDE.md/INTERFACES→更新接头总览（详见 `WoWorld-Design/Change/` 中 CHG-025~028 实践） |
