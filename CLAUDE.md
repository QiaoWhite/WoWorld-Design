...
| **看最新开发日志** | `woworld-dev-plan/01-核心基础/devlogs/DEVLOG-2026-07-06.md` (Sprint 032-033: VoxelChunk 修复 + LODCoordinator + 天气 + PBR) |
| **看最新交接文档** | `woworld-dev-plan/01-核心基础/handoff/handoff-20260705-026.md` |
...
近期冲刺：**Sprint 031**（性能优化3轮+海洋视觉+VoxelChunk LOD 0替换）→ **Sprint 032**（VoxelChunk LOD 0 7轮修复·MC绕序+统一wy+biome材质）→ **Sprint 033**（LODCoordinator Phase1+天气Phase1+PBR法线修复·84测试）。
...
项目已进入轨A·GPU-Driven Clipmap+海洋+LODCoordinator+天气+**ECS Phase 0 待启动**。Rust workspace 含 4 crate（`woworld_ecs` 规划中），**107 个测试全部通过**。
...
# 运行所有测试
cargo test --workspace           # 4 crates，全部 107 个测试
...
> **当前状态**（2026-07-05）：`cargo check --workspace` 通过。`cargo test` 107 个测试全部通过, clippy 零警告。GPU-Driven Clipmap 8 层 LOD + Gerstner 海洋 + 昼夜循环 + 5 群系系统 + OceanProvider trait + Transvoxel 骨架 + **LODCoordinator Phase2（完整8步算法）** + **天气系统 Phase1** 就位。VoxelChunk LOD 0 渲染修复完成。**ECS 架构已采纳（hecs）——`woworld_ecs` crate 规划中——详见 `开发文档/`**。最新状态见 `woworld-dev-plan/01-核心基础/devlogs/`。

### 测试分布

| Crate | 测试数 | 说明 |
|-------|--------|------|
| `woworld_core` | 41 | time + density + lod + weather_types |
| `woworld_worldgen` | 49 | biome + cave + clipmap + noise_gen + ocean + terrain + transvoxel |
| `woworld_atmosphere` | 17 | time_curve + synthesizer |
| `woworld_godot` | 0 | cdylib 不便于单元测试——已迁移至 worldgen |
...
- **`woworld_ecs` — ECS Component 定义（规划中·待创建）**：所有 ECS Component（`Position`, `Rotation`, `Velocity`, `EntityKind`, `LodLevel` 等）将在此定义。依赖 `woworld_core`（值类型 + trait）+ `hecs`（Archetype SoA 存储）。不含 System 逻辑——System 实现在各自所属 crate。Component 当前暂驻各消费 crate，待 `woworld_ecs` 创建后迁移。
