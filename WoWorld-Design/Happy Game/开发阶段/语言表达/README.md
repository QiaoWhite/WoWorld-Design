...
**★ v1.1 新增**: 修辞化渲染——TextGenerator 在 `render_utterance()` 中检测跨域概念对并触发 simile/metaphor/analogical explanation 三种修辞装置。零新 crate / 零新 trait，纯数据驱动。消费 NPC 认知系统的 `domain_sig` 和 `woworld_core::domain_similarity_raw()`。
...
| **v1.1** | 信息传播系统（五通道+失真+谣言生命周期+NPC间对话渲染+悄悄话/密谋）+ 非语言表达联动 + 对话→记忆消化 + 对话中断/恢复 + 文化沟通规范 + Writeable trait + ConversationSnapshot **+ ★ 修辞化渲染（CHG-057: simile/metaphor/analogical explanation · consumption of domain_sig + domain_similarity_raw）** |
...
| 项目 | 数值 |
|------|------|
| TextGenerator 热路径 | < 0.01ms/调用——片段索引 O(1) HashMap |
| 对话系统参与者上限 | 2-1000+ (SocialField 批量处理) |
| 信息传播通道 | 5 通道并行——独立失真算子 |
| LLM 增强 | 可选——~1% 对话场景触发 |
| ExpressionRegistry | ~430 文本片段 + ~50 标准参数——全部 TOML |

> **关联**: [[001-语言表达系统总纲]] · [[../技术栈方案/001-WoWorld正式技术栈方案v3|技术栈方案 v4.0]] · [[../NPC活人感模块/NPC活人感开发文档ver2.0|NPC 活人感 ver2.0]] · [[../历史/001-历史系统总纲|历史系统]] · [[../魔法/01-基础层/001-魔法总纲|魔法系统]] · [[../物品系统/001-物品系统总纲|物品系统]] · [[../技能系统/001-技能系统总纲|技能系统]]
> **设计哲学**: 语言表达模块不拥有内容——它让内容被感知。ContentResolver trait 将内容的定义权完全留给各业务模块。语言模块只负责"如何说"，不负责"说什么"。
