...
| **008** | [跨模块接口与数据合同](008-跨模块接口与数据合同.md) | 全部 trait 汇集、各模块 impl 规范、LMDB key 约定、性能预算汇总、安全边界、ReliabilityHint、版本路线 |
...
| **v1.1** | Writeable trait + ConversationSnapshot + CopyComparison 整合 |
| **v1.2** | L2 嵌入向量匹配 + Godot 语音集成规范 + SensoryMapping 实现 |
| **v2.0** | L3 LLM 集成 + 信息传播系统对接 |
