...
impl Default for RelationshipNorms {
    fn default() -> Self {
        Self {
            monogamous_default: true,
            polygyny_acceptance: 0.0,
            polyandry_acceptance: 0.0,
            polyamory_acceptance: 0.0,
            group_marriage_acceptance: 0.0,
            arranged_marriage_tendency: 0.5,
            contractual_marriage_acceptance: 0.0,
            homosexuality_acceptance: 0.5,
            same_sex_marriage_legal: 0.0,
            conjugal_birth_norm: 0.5,
            divorce_acceptance: 0.5,
            max_spouses: None,
            residence_pattern: ResidencePattern::default(),
        }
    }
}
...
impl RelationshipNorms {
    ...
    pub fn derive_from(core: &CultureCoreParams) -> Self {
        ...
        let polygyny_acceptance =
            (hierarchy * 0.8 + core.militarism * 0.2 - core.individualism * 0.4).clamp(0.0, 1.0);

        // polyandry = (1-power_distance) × 0.3 + long_term × 0.1 - religiosity × 0.3, cap 0.3
        let polyandry_acceptance = ((1.0 - core.power_distance) * 0.3
            + core.long_term_orientation * 0.1
            - core.religiosity * 0.3)
            .clamp(0.0, 1.0)
            .min(0.3);
        ...
        let polyamory_acceptance = (core.individualism * 0.5 + core.indulgence * 0.3
            ...
            - core.uncertainty_avoidance * 0.2)
            .clamp(0.0, 1.0)
            .min(0.4);
        ...
        let contractual_marriage_acceptance =
            (core.individualism * 0.4 + core.long_term_orientation * 0.3 - core.religiosity * 0.5)
                .clamp(0.0, 1.0)
                .min(0.6);
        ...
        let homosexuality_acceptance = (core.individualism * 0.6
            + core.openness_to_outsiders * 0.2
            - core.religiosity * 0.5
            - core.uncertainty_avoidance * 0.3
            - core.power_distance * 0.3)
            .clamp(0.0, 1.0);
        ...
        let conjugal_birth_norm =
            (core.religiosity * 0.6 + core.power_distance * 0.3 + core.uncertainty_avoidance * 0.2
                - core.individualism * 0.3)
                .clamp(0.0, 1.0);
        ...
        let divorce_acceptance = (core.individualism * 0.5 + core.indulgence * 0.2
            - core.religiosity * 0.6
            - core.uncertainty_avoidance * 0.3
            - core.long_term_orientation * 0.2)
            .clamp(0.0, 1.0);
        ...
        Self {
            monogamous_default,
            polygyny_acceptance,
            polyandry_acceptance,
            polyamory_acceptance,
            group_marriage_acceptance,
            arranged_marriage_tendency,
            contractual_marriage_acceptance,
            homosexuality_acceptance,
...
mod tests {
    ...
    fn test_all_fields_in_range() {
        for seed in 0..100 {
            ...
            if let Some(ms) = n.max_spouses {
                assert!((2..=8).contains(&ms));
            }
        }
    }
