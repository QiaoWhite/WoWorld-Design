...
impl CognitiveStyle {
    ...
    pub fn derive_from_bigfive(b: &BigFive) -> Self {
        ...
        let ri = ((1.0 - b.neuroticism) * 0.5 + b.openness * 0.3 + (1.0 - b.extraversion) * 0.2)
            .clamp(0.0, 1.0);
        ...
        let rf = (b.openness * 0.6 + (1.0 - b.conscientiousness) * 0.2 + 0.5 * 0.2).clamp(0.0, 1.0);
...
mod tests {
    ...
    fn test_abstract_open_high() {
        ...
        let cs = CognitiveStyle::derive_from_bigfive(&b);
        assert!(
            cs.abstract_concrete > 0.65,
            "high openness → abstract, got {}",
            cs.abstract_concrete
        );
    }
    ...
    fn test_rigid_open_flexible() {
        ...
        let cs = CognitiveStyle::derive_from_bigfive(&b);
        assert!(
            cs.rigid_flexible > 0.8,
            "high O low C → flexible, got {}",
            cs.rigid_flexible
        );
    }
    ...
    fn test_rigid_conservative_rigid() {
        ...
        let cs = CognitiveStyle::derive_from_bigfive(&b);
        assert!(
            cs.rigid_flexible < 0.2,
            "low O high C → rigid, got {}",
            cs.rigid_flexible
        );
    }
    ...
    fn test_all_dimensions_in_range() {
        ...
        for seed in 0..20 {
            ...
            let cs = CognitiveStyle::derive_from_bigfive(&b);
            assert!(
                (0.0..=1.0).contains(&cs.analytic_intuitive),
                "seed {seed}: ai out of range"
            );
            assert!(
                (0.0..=1.0).contains(&cs.reflective_impulsive),
                "seed {seed}: ri out of range"
            );
            assert!(
                (0.0..=1.0).contains(&cs.abstract_concrete),
                "seed {seed}: ac out of range"
            );
            assert!(
                (0.0..=1.0).contains(&cs.rigid_flexible),
                "seed {seed}: rf out of range"
            );
        }
    }
}
