...
pub struct CultureRegistry {
    ...
    relationship_norms: Vec<RelationshipNorms>,
    valid: Vec<bool>,
    culture_ids: Vec<CultureId>,
    next_id: u32,
}
...
impl CultureRegistry {
    ...
    pub fn register(&mut self, params: CultureCoreParams) -> CultureId {
        ...
        let id = CultureId(self.next_id);
        self.next_id += 1;
        let comm = CommunicationNorms::derive_from(&params);
        let building = BuildingStylePreferences::derive_from(
        ...
        self.cores.push(params);
        self.communication_norms.push(comm);
        self.building_styles.push(building);
        self.beauty_standards.push(beauty);
        self.fertility_norms.push(fertility);
        self.dietary_prefs.push(dietary);
        self.relationship_norms.push(relationship);
        self.valid.push(true);
        self.culture_ids.push(id);
        id
    }

    #[allow(clippy::too_many_arguments)]
    pub fn register_with_biome(
        &mut self,
    ...
    pub fn get_params_mut(&mut self, id: CultureId) -> Option<&mut CultureCoreParams> {
        ...
        if idx < self.cores.len() && self.valid[idx] {
    ...
    pub fn recalculate_derived(
        ...
        self.building_styles[idx] = BuildingStylePreferences::derive_from(
            core,
            is_arid,
            is_arctic,
            is_forested,
...
impl CultureQuery for CultureRegistry {
    ...
    fn communication_norms(&self, id: CultureId) -> Option<&CommunicationNorms> {
        ...
        let idx = id.0 as usize;
        if idx < self.communication_norms.len() && self.valid[idx] {
            Some(&self.communication_norms[idx])
        } else {
            None
        }
    }
    fn building_style(&self, id: CultureId) -> Option<&BuildingStylePreferences> {
        ...
        let idx = id.0 as usize;
        if idx < self.building_styles.len() && self.valid[idx] {
            Some(&self.building_styles[idx])
        } else {
            None
        }
    }
    fn beauty_standard(&self, id: CultureId) -> Option<&CulturalBeautyStandard> {
        ...
        let idx = id.0 as usize;
        if idx < self.beauty_standards.len() && self.valid[idx] {
            Some(&self.beauty_standards[idx])
        } else {
            None
        }
    }
    fn fertility_norms(&self, id: CultureId) -> Option<&FertilityNorms> {
        ...
        let idx = id.0 as usize;
        if idx < self.fertility_norms.len() && self.valid[idx] {
            Some(&self.fertility_norms[idx])
        } else {
            None
        }
    }
    fn dietary_preferences(&self, id: CultureId) -> Option<&DietaryBasePreferences> {
        ...
        let idx = id.0 as usize;
        if idx < self.dietary_prefs.len() && self.valid[idx] {
            Some(&self.dietary_prefs[idx])
        } else {
            None
        }
    }
    fn relationship_norms(&self, id: CultureId) -> Option<&RelationshipNorms> {
        ...
        let idx = id.0 as usize;
        if idx < self.relationship_norms.len() && self.valid[idx] {
            Some(&self.relationship_norms[idx])
        } else {
            None
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_new_empty() {
        let r = CultureRegistry::new();
        assert_eq!(r.len(), 0);
    }
    #[test]
    fn test_register_ids() {
        let mut r = CultureRegistry::new();
        assert_eq!(r.register(CultureCoreParams::from_seed(0)), CultureId(0));
    }
    #[test]
    fn test_query() {
        let mut r = CultureRegistry::new();
        let id = r.register(CultureCoreParams::from_seed(42));
        assert!(r.core_params(id).is_some());
    }
    #[test]
    fn test_register_with_biome() {
        let mut r = CultureRegistry::new();
        r.register_with_biome(
            CultureCoreParams::from_seed(0),
            true,
            false,
            true,
            false,
    ...
    fn test_recalculate() {
        let mut r = CultureRegistry::new();
        let id = r.register(CultureCoreParams::from_seed(0));
        r.recalculate_derived(id, false, false, false, false, false, false, false, false);
    }
    #[test]
    fn test_none_id() {
        let mut r = CultureRegistry::new();
        r.register(CultureCoreParams::from_seed(0));
        assert!(r.core_params(CULTURE_ID_NONE).is_none());
    }
}
