...
use woworld_core::faith::{
    derive_faith_label, FaithId, FaithLabel, FaithQuery, FaithTheology,
    ReligiousPracticeProfile, FAITH_ID_NONE,
};
...
impl FaithRegistry {
    ...
    pub fn get_profile(&self, entity_id: EntityId) -> Option<&ReligiousPracticeProfile> {
        self.profiles.iter().find(|(e, _)| *e == entity_id).map(|(_, p)| p)
    }

    /// 获取可变神学参数引用（用于 Phase 2 漂移）
    pub fn get_theology_mut(&mut self, id: FaithId) -> Option<&mut FaithTheology> {
        let idx = id.0 as usize;
        if idx < self.theologies.len() { Some(&mut self.theologies[idx]) } else { None }
    }
    ...
    pub fn len(&self) -> usize { self.faith_ids.len() }
    pub fn is_empty(&self) -> bool { self.faith_ids.is_empty() }
...
impl FaithQuery for FaithRegistry {
    fn theology(&self, id: FaithId) -> Option<&FaithTheology> {
        if id == FAITH_ID_NONE { return None; }
        let idx = id.0 as usize;
        if idx < self.theologies.len() { Some(&self.theologies[idx]) } else { None }
    }

    fn faith_label(&self, id: FaithId) -> Option<FaithLabel> {
        if id == FAITH_ID_NONE { return None; }
        let idx = id.0 as usize;
        if idx < self.labels.len() { Some(self.labels[idx]) } else { None }
    }
    ...
    fn all_faiths(&self) -> &[FaithId] { &self.faith_ids }

    fn faith_count(&self) -> usize { self.faith_ids.len() }
}
...
mod tests {
    ...
    fn test_proselytizing_delegates() {
        let mut r = FaithRegistry::new();
        let t = FaithTheology { exclusivity: 0.9, orthodoxy_vs_orthopraxy: 0.5, faith_as_identity: 0.5, ..FaithTheology::default() };
        let id = r.register(t);
    ...
    fn test_tolerance_hostility() {
        let mut r = FaithRegistry::new();
        let a = r.register(FaithTheology { exclusivity: 0.1, ..FaithTheology::from_seed(10, 0.7) });
        let b = r.register(FaithTheology { exclusivity: 0.9, ..FaithTheology::from_seed(20, 0.7) });
    ...
    fn test_profile_update() {
        let mut r = FaithRegistry::new();
        r.set_profile(EntityId(1), ReligiousPracticeProfile::new(ReligiousMotivation::SocialCustom));
        r.set_profile(EntityId(1), ReligiousPracticeProfile::new(ReligiousMotivation::PersonalDevotion));
        assert_eq!(r.profile_count(), 1);
        assert_eq!(r.get_profile(EntityId(1)).unwrap().motivation, ReligiousMotivation::PersonalDevotion);
    }
    ...
    fn test_recalculate_label() {
        ...
        assert_eq!(r.faith_label(id), Some(FaithLabel::Monotheism(MonotheismFlavor::Standard)));
        // 修改神学 → NonTheistic
