...
fn interpolate(p1: Vec3, p2: Vec3, d1: f32, d2: f32) -> Vec3 {
    if (d1 - ISOVALUE).abs() < f32::EPSILON {
        return p1;
    }
    if (d2 - ISOVALUE).abs() < f32::EPSILON {
        return p2;
    }
    if d1 == d2 {
        return (p1 + p2) * 0.5;
...
fn global_corners(ix: usize, iy: usize, iz: usize, stride_x: usize, stride_y: usize) -> [u32; 8] {
    ...
    [
        base as u32,
        (base + 1) as u32,
        (base + 1 + stride_y) as u32,
        (base + stride_y) as u32,
        (base + stride_x) as u32,
        (base + 1 + stride_x) as u32,
        (base + 1 + stride_x + stride_y) as u32,
        (base + stride_x + stride_y) as u32,
    ]
}
...
fn extract_transition_cell(
    ...
    let corner_positions: [Vec3; 13] = match face {
        0 => [
            ...
            Vec3::new(min_x - half_vs, min_y, max_z),
            Vec3::new(min_x - half_vs, min_y, min_z),
            Vec3::new(min_x - half_vs, max_y, max_z),
            Vec3::new(min_x - half_vs, max_y, min_z),
            Vec3::new(min_x - half_vs, mid_y, mid_z),
            Vec3::new(min_x, min_y, mid_z),
            Vec3::new(min_x, mid_y, min_z),
            Vec3::new(min_x, max_y, mid_z),
            Vec3::new(min_x, mid_y, max_z),
        ],
        1 => [
            // +X
            ...
            Vec3::new(max_x + half_vs, min_y, min_z),
            Vec3::new(max_x + half_vs, min_y, max_z),
            Vec3::new(max_x + half_vs, max_y, min_z),
            Vec3::new(max_x + half_vs, max_y, max_z),
            Vec3::new(max_x + half_vs, mid_y, mid_z),
            Vec3::new(max_x, min_y, mid_z),
            Vec3::new(max_x, mid_y, max_z),
            Vec3::new(max_x, max_y, mid_z),
            Vec3::new(max_x, mid_y, min_z),
        ],
        2 => [
            // -Z
            Vec3::new(max_x, min_y, min_z),
            ...
            Vec3::new(max_x, max_y, min_z),
            Vec3::new(min_x, max_y, min_z),
            Vec3::new(max_x, min_y, min_z - half_vs),
            Vec3::new(min_x, min_y, min_z - half_vs),
            Vec3::new(max_x, max_y, min_z - half_vs),
            Vec3::new(min_x, max_y, min_z - half_vs),
            Vec3::new(mid_x, mid_y, min_z - half_vs),
            Vec3::new(mid_x, min_y, min_z),
            Vec3::new(min_x, mid_y, min_z),
            Vec3::new(mid_x, max_y, min_z),
            Vec3::new(max_x, mid_y, min_z),
        ],
        3 => [
            // +Z
            ...
            Vec3::new(min_x, max_y, max_z),
            Vec3::new(max_x, max_y, max_z),
            Vec3::new(min_x, min_y, max_z + half_vs),
            Vec3::new(max_x, min_y, max_z + half_vs),
            Vec3::new(min_x, max_y, max_z + half_vs),
            Vec3::new(max_x, max_y, max_z + half_vs),
            Vec3::new(mid_x, mid_y, max_z + half_vs),
            Vec3::new(mid_x, min_y, max_z),
            Vec3::new(max_x, mid_y, max_z),
            Vec3::new(mid_x, max_y, max_z),
            Vec3::new(min_x, mid_y, max_z),
        ],
        _ => return,
    };
    ...
    for (i, p) in corner_positions.iter().enumerate().take(9) {
        let d = stack.density_at(WorldPos {
            x: p.x as f64,
            y: p.y as f64,
            z: p.z as f64,
        });
    ...
    for &edge_code in vert_data.iter().take(vertex_count) {
        ...
        let d_a = stack.density_at(WorldPos {
            x: p_a.x as f64,
            y: p_a.y as f64,
            z: p_a.z as f64,
        });
        let d_b = stack.density_at(WorldPos {
            x: p_b.x as f64,
            y: p_b.y as f64,
            z: p_b.z as f64,
        });
        ...
        let mat_id = base_layer.material_at(WorldPos {
            x: pos.x as f64,
            y: pos.y as f64,
            z: pos.z as f64,
        });
    ...
    for t in 0..triangle_count {
        ...
        if i0 >= face_vert_indices.len()
            || i1 >= face_vert_indices.len()
            || i2 >= face_vert_indices.len()
        {
            continue;
        }
...
pub fn transvoxel_extract(
            ...
            for ix in 0..sx {
                ...
                for e in 0..12 {
                    ...
                    if let Some(&vi) = edge_cache.get(&key) {
                        ...
                        let n =
                            gradient_from_stack(stack, pos.x as f64, pos.y as f64, pos.z as f64);
                        let mat_id = base_layer.material_at(WorldPos {
                            x: pos.x as f64,
                            y: pos.y as f64,
                            z: pos.z as f64,
    ...
    for face_bit in 0..4u8 {
        if transition_faces & (1 << face_bit) == 0 {
            continue;
        }
        ...
        match face_bit {
            0 => {
                ...
                for iz in 0..sz {
                    for iy in 0..sy {
                        let cx = ox;
                        let cy = oy + iy as f64 * voxel_size;
                        let cz = oz + iz as f64 * voxel_size;
            ...
            1 => {
                ...
                for iz in 0..sz {
                    for iy in 0..sy {
                        let cx = ox + (sx as f64 - 2.0) * voxel_size;
                        let cy = oy + iy as f64 * voxel_size;
                        let cz = oz + iz as f64 * voxel_size;
                        if sx < 2 {
                            continue;
            ...
            2 => {
                ...
                for ix in 0..sx {
                    for iy in 0..sy {
                        let cx = ox + ix as f64 * voxel_size;
                        let cy = oy + iy as f64 * voxel_size;
                        let cz = oz;
                        extract_transition_cell(
                            2,
            ...
            3 => {
                ...
                for ix in 0..sx {
                    for iy in 0..sy {
                        let cx = ox + ix as f64 * voxel_size;
                        let cy = oy + iy as f64 * voxel_size;
                        let cz = oz + (sz as f64 - 2.0) * voxel_size;
                        if sz < 2 {
                            continue;
                        }
                        extract_transition_cell(
...
mod tests {
    ...
    fn test_empty_stack_returns_empty_mesh() {
        ...
        assert!(
            ...
            "all-negative density should produce no vertices"
        );
    ...
    fn test_solid_sphere_produces_surface() {
        let mut stack = DensityStack::new();
        let sphere = Arc::new(SphereDensity {
            cx: 4.0,
            cy: 4.0,
            cz: 4.0,
            r: 3.0,
        ...
        assert!(
            ...
            "sphere should produce at least 50 vertices, got {}",
            mesh.vertices.len()
        );
        assert!(mesh.indices.len() >= 3);
        // 所有顶点应在球面上: |p - c| ≈ r
        for v in &mesh.vertices {
            let d = ((v.x as f64 - 4.0).powi(2)
                + (v.y as f64 - 4.0).powi(2)
                + (v.z as f64 - 4.0).powi(2))
            .sqrt();
            assert!(
                (d - 3.0).abs() < 0.7,
                "vertex distance from center should be ~3.0, got {d} at {:?}",
                v
            );
        }
    }
    ...
    fn test_deterministic_output() {
        ...
        let sphere_a = Arc::new(SphereDensity {
            cx: 4.0,
            cy: 4.0,
            cz: 4.0,
            r: 2.0,
        ...
        let colors = HashMap::new();
        let mesh_a = transvoxel_extract(
            &stack_a, &*sphere_a, 0.0, 0.0, 0.0, 8, 8, 8, 1.0, 0, &colors,
        );

        let mut stack_b = DensityStack::new();
        let sphere_b = Arc::new(SphereDensity {
            cx: 4.0,
            cy: 4.0,
            cz: 4.0,
            r: 2.0,
        ...
        let mesh_b = transvoxel_extract(
            &stack_b, &*sphere_b, 0.0, 0.0, 0.0, 8, 8, 8, 1.0, 0, &colors,
        );
        ...
        for i in 0..mesh_a.vertices.len() {
            let va = mesh_a.vertices[i];
            let vb = mesh_b.vertices[i];
            assert!(
                (va.x - vb.x).abs() < 0.001
                    && (va.y - vb.y).abs() < 0.001
                    && (va.z - vb.z).abs() < 0.001,
    ...
    fn test_edge_cache_vertex_sharing() {
        ...
        assert!(
            unique_vertices < max_if_unshared,
